...
def main():
    ...
    order = ["mfs_nocache", "mfs_cache", "ext4"]
    labels = {"mfs_nocache": "mfs (no cache)", "mfs_cache": "mfs (cache)", "ext4": "ext4"}
    colors = {"mfs_nocache": "#e76f51", "mfs_cache": "#2a9d8f", "ext4": "#264653"}
    data = {t: {r["op"]: float(r["ops_per_s"]) for r in rows if r["target"] == t} for t in order}

    x = np.arange(len(ops)); w = 0.26
    fig, ax = plt.subplots(figsize=(8, 4.5))
    for i, t in enumerate(order):
        ax.bar(x + (i-1)*w, [data[t].get(o, 0) for o in ops], w, label=labels[t], color=colors[t])
